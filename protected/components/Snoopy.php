...
class Snoopy
{
	const SNOOPY_SOCKET_CREATE_FAILED = -3;	//socket creation failed
	const SNOOPY_SOCKET_DNS_FAILED = -4;	//dns lookup failure
	const SNOOPY_SOCKET_CONNECT_REFUSED = -5;
	const SNOOPY_TIMEOUT = -100;
	const SNOOPY_SOCKET_UNKOWN_ADDRESSINFO = -200;
	...
	public $agent			=	"Mozilla/5.0 (Windows NT 6.1; rv:2.0) Gecko/20110319 Firefox/4.0";	// agent we masquerade as
	...
	public	$maxlength	= 50000000;				// max return data length (body)
		...
		try {
			...
			switch($this->status)
			{
				...
				case -5:
					$this->error="connection refused or timed out (-5)";
				default:
					$this->error="connection failed (".$e->getCode().")";
			}
			return false;
		}
	}
